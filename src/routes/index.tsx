...
function Home() {
  ...
  return (
    <div className="flex min-h-[calc(100vh-3rem)] w-full items-center justify-center p-4">
      <div className="flex w-full items-center justify-center">
        <div className="text-center">
          <h1 className="text-4xl font-bold tracking-tight">SolvexR</h1>
          ...
          <div className="my-4 w-[30rem] max-w-full rounded-2xl border-[1.5px] border-dashed border-[var(--border-color)] bg-transparent px-0 pb-[15px] pt-2">
            ...
            <div className="mb-2 border-t border-dashed border-[var(--border-color)] text-sm">
              <div className="flex items-center justify-between border-b border-dashed border-[var(--border-color)] p-[10px]">
                <div className="flex items-center gap-1">
                  <Timer size={20} className="mr-1" /> Time Control:
                </div>
                <b className="text-[13px] font-normal not-italic text-[#afafaf]">{timeControl} min</b>
              </div>
              <div className="flex items-center justify-between border-b border-dashed border-[var(--border-color)] p-[10px]">
                <div className="flex items-center gap-1">
                  <Calculator size={20} className="mr-1" /> Operations Selection:
                </div>
                <b className="text-[13px] font-normal not-italic text-[#afafaf]">{printOperations(newOperations)}</b>
              </div>
              <div className="flex items-center justify-between border-b border-dashed border-[var(--border-color)] p-[10px]">
                <div className="flex items-center gap-1">
                  <ArrowsLeftRight size={20} className="mr-1" /> Ranges for Numbers:
                </div>
                <b className="text-[13px] font-normal not-italic text-[#afafaf]">{`1: 1-${numberOneRange}, 2: 1-${numberTwoRange}`}</b>
              </div>
            </div>
            <b className="text-[13px] font-normal not-italic text-[#afafaf]">
              {`-> Go to `} <Link to="/settings" className="cursor-pointer text-white underline">Settings</Link> to change any of the above.
            </b>
          </div>
          {containsDivision && (
            <div className="mx-auto mb-6 w-[30rem] max-w-full overflow-hidden rounded-xl bg-[#1e1f22] p-4 text-left shadow-sm ring-1 ring-white/[0.06]">
          ...
          <Link to="/app" aria-label="Start practicing" className="learn-more-teal">
            START
          </Link>
        </div>
      </div>
